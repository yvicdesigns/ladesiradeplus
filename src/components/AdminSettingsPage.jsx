...
import { AdminResetDataPage } from '@/components/AdminResetDataPage'; 
...
export const AdminSettingsPage = () => {
  ...
  // Flatten tab logic to single parameter 'sub' for simplicity and robust routing
  const tabParam = searchParams.get('sub') || searchParams.get('tab') || searchParams.get('main');
  const currentTab = tabParam || 'profile';

  // Task 6: Add console logging to verify routing logic
  useEffect(() => {
    console.log("[AdminSettingsPage] Current Tab resolved to:", currentTab, "from URL params sub:", searchParams.get('sub'), "tab:", searchParams.get('tab'));
  ...
  const checkInitializationStatus = async () => {
    ...
    try {
      ...
      setInitStatus(data || { success: true, initialized: false, count: 0, restaurant_id: null });
    } catch (err) {
      console.error("Failed to check settings status:", err);
      setInitStatus({ success: false, initialized: false, count: 0, restaurant_id: null });
    } finally {
      setIsChecking(false);
    }
  };
  ...
  const handleTabChange = (value) => {
    setSearchParams({ sub: value }, { replace: true });
  };
        ...
        <div className="max-w-2xl mx-auto mt-12 space-y-6">
          <Card className="border-amber-200 bg-amber-50">
            <CardHeader className="text-center pb-2">
              <Wrench className="h-12 w-12 mx-auto text-amber-50 mb-2" />
      ...
      <div className="space-y-4 md:space-y-6 landscape:space-y-2 pb-12 landscape:pb-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 landscape:gap-2">
          <div>
            <h1 className="text-2xl md:text-3xl landscape:text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
              Configuration Système <Lock className="h-5 w-5 text-primary opacity-50"/>
            </h1>
            <p className="text-muted-foreground text-sm landscape:text-xs">Gérez les préférences globales et les outils d'administration.</p>
          </div>
        ...
        <Tabs value={currentTab} onValueChange={handleTabChange} className="w-full space-y-4">
          <div className="w-full border rounded-lg bg-card/50 shadow-sm">
            <ScrollArea className="w-full max-w-[calc(100vw-32px)] lg:max-w-[calc(100vw-280px)]">
              <TabsList className="inline-flex w-max min-w-full justify-start h-12 p-1 bg-transparent">
                ...
                <TabsTrigger value="promo-banner" className="font-medium text-primary">Bannières Promo</TabsTrigger>
                <TabsTrigger value="notifications">Notifications</TabsTrigger>
                <TabsTrigger value="sound">Sons</TabsTrigger>
                <TabsTrigger value="delivery-alerts">Livraison</TabsTrigger>
                <TabsTrigger value="users">Utilisateurs</TabsTrigger>
                <TabsTrigger value="mobile-money">Paiements Mobiles</TabsTrigger>
                <TabsTrigger value="security">Sécurité</TabsTrigger>
                <TabsTrigger value="data">Données</TabsTrigger>
                <TabsTrigger value="reset">Réinitialisation Data</TabsTrigger>
              </TabsList>
          ...
          <div className="bg-card rounded-xl border border-border/50 shadow-sm relative min-h-[400px] p-4 md:p-6">
            ...
            <TabsContent value="promo-banner" className="m-0 animate-in fade-in duration-300"><PromobannerTab /></TabsContent>
            ...
            <TabsContent value="reset" className="m-0"><AdminResetDataPage /></TabsContent>
          </div>
        </Tabs>
      </div>
    </AdminLayout>
